...
// Evaluate appearance error (original color mode)
//
// Usage:
//   distances = evaluateEaColorMex(tmp_y, tmp_cb, tmp_cr, ...
//                                  img_y, img_cb, img_cr, ...
...
#pragma omp parallel for private(i) num_threads(8)
    for (i = 0; i < num_poses; i++) {
        ...
        for (int j = 0; j < num_points; j++) {
            ...
            score += 0.50*fabs(tmp_y_vals[j] - img_y[img_index])
                   + 0.25*fabs(tmp_cb_vals[j] - img_cb[img_index])
                   + 0.25*fabs(tmp_cr_vals[j] - img_cr[img_index]);
        }
